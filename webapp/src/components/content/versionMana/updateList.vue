  ...
  <div class="ver-update-list">
	...
	<el-table
	    ...
	    <el-table-column
	      ...
	      <template slot-scope="scope">
	        <el-popover trigger="hover" placement="top">
	          ...
	          <div slot="reference" class="name-wrapper" v-if="scope.row.apkurl.length > 0">
	    ...
	    <el-table-column
	      label="更新说明">
	      <template slot-scope="scope">
	        <el-popover trigger="hover" placement="top" v-if="scope.row.updatemessage.length > 0">
	          <p>{{ scope.row.updatemessage }}</p>
	          <div slot="reference" class="name-wrapper">
	            <span style="margin-left: 10px">{{ scope.row.updatemessage }}</span>
	          </div>
	        </el-popover>
	      </template>
	    </el-table-column>
...
<script>
	export default {
	  	...
	  	methods: {
	  	  getTableData () {
	  	  	this.addLoad = true;
	  	  	this.$http.post('http://upgrate.block18.io:9999/api/getHotUpdateList',{
